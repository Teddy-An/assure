...
| 버전 기준 | 값 |
|---|---|
| Assure 정책 버전 | `0.2.0-dev` |
| 검증 정책 | `assure-generated-probes-v2` |
| Probe 스키마 | `2` |
| 생성기 계약 | `assure-llm-probe-v2` |

설치본은 `<정책 버전>+codex.<cachebuster>` 형식으로 표시됩니다. cachebuster는
Codex가 업데이트된 로컬 플러그인을 다시 읽도록 만드는 설치 식별자이며,
정책 버전은 `0.2.0-dev`입니다. 실행 시에는 스크립트·스킬 내용으로 배포
SHA-256을 계산해 생성된 probe에 기록하고 현재 설치본과 비교합니다.
...
Assure의 승인과 실행 흐름은 다음 세 규칙을 반드시 지킵니다.

1. **승인은 테스트 전에 한 번만 받습니다.**
   환경 조사 후 기술 스택, 의존성 설치, 권한, Sandbox 구조, 생성·교체할
   `.assure/` 파일, probe와 Adapter, 실행 명령 및 정리 절차를 하나의
   계획으로 보여줍니다. 사용자가 승인하면 Sandbox 구성과 테스트 준비에
   필요한 전체 절차가 승인된 것입니다. 이후 개별 설치나 파일 생성 승인을
   다시 요청하지 않습니다.
2. **과거 정책의 probe는 강제로 교체합니다.**
   현재 Assure 버전·배포 해시·probe 스키마·생성기 계약과 다르면
   `.assure/probes/` 아래의 오래된 Assure 소유 파일을 별도 질문 없이
   삭제하고 최신 정책으로 remap하여 재생성합니다. 프로젝트 소유 테스트는
   변경하거나 삭제하지 않습니다.
3. **테스트가 시작되면 결과까지 멈추지 않습니다.**
   의존성 설치, 권한 확인, probe 생성과 Sandbox 건강검사는 모두 테스트
   시작 전에 끝냅니다. 테스트 중에는 승인이나 선택을 요청하지 않으며,
   각 항목을 `통과`, `실패`, `미검증`, `확인` 중 하나로 기록해 최종
   보고서까지 진행합니다.
...
기능 probe 정책은 이름만 기록해 통과시킬 수 없습니다. Assure는
